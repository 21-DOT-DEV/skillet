...
2. **The harness matrix.** `skillet run --harness claude-code,opencode` executes the same suite through multiple agents and prints a per-harness pass^k portability table — "passes on Claude Code, flakes on OpenCode." No existing tool offers this, and only a design that is multi-harness from day one can.
...
Two notes the table surfaces. First, **name collisions**: two public tools already ship a `skill-eval` binary (AWS's audit CLI and mgechev's framework), and a follow-up check found `skillet` equally occupied — Palo Alto Networks ships an entire Skillet product family (skilletlib, SLI, skilletcli, IronSkillet) — see Open Question 1. Second, the **harness surface keeps widening**: Gemini CLI now supports Agent Skills with an explicit `activate_skill` step, making it both a future adapter candidate and an easy trigger-axis signal source.
