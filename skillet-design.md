...
| `skillet harness which <id> [--search <root>]` | Print the resolved binary as an `export`-able pin; `--search` is the *opt-in* recursive vendored-copy finder (newest-mtime-first). Adapters never auto-probe other apps' private caches (§9.1) |
