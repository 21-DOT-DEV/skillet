...
- CI: `.github/workflows/ci.yml` runs the free suite on macOS (`macos-26`, `DEVELOPER_DIR` pinned to
  Xcode 26.4 — the swift-bitcoinkernel convention; swift-yaml's C++ interop needs the Swift 6.3
  toolchain) + Ubuntu (official `swift:6.3` container) on every push/PR — zero secrets, zero paid
  calls; the live smoke self-skips (opt-in locally via `SKILLET_LIVE_SMOKE=1`).
