...
MINOR — **F29 SHIPPED: structured friction & finding evidence + lifecycle** ([Specs/015](Specs/015-friction-finding-evidence/plan.md); design → v0.52). The `skillet.friction/1` (human) + `skillet.finding/1` (machine-mined) frontmatter formats over a shared, composed `EvidenceHeader`, an `Evidence` protocol the later gates engine reads uniformly, and a typed `LifecycleState` + centralized legal-moves validator (`logged → candidate → codified → proven → closed` + `watch`/`held` side states + the one constrained `closed → logged` reopen; illegal/self transitions rejected — "never implicit" is a unit-tested property of `EDDCore`). New pure-core tree `EDDCore/Evidence/*` (models, validation, lifecycle, errors) + the `ConfigYAML` `EvidenceFrontmatter` seam (decode/validate + create-encode) with **native duplicate-key rejection** (`DuplicateKeyStrategy.reject`) and a shared `FrontmatterSplit` scan. Re-scoped mid-flight to **Path B** (D6) after `swift-yaml` shipped in-place editing + native `.reject` — dropping the whole Path-A neutral-map round-trip stack; the pin was bumped (`048f714` → `473252b`) as an isolated verified step. **Library layer only** — the `friction add`/`set-state`/`next` commands (F30) and the gates consumer build on this; no new CLI surface yet. 511 tests green; staged, pending commit. Phase 3 remains IN PROGRESS (F27/F28/F30/F31 remain).

## v1.18.0 — 2026-07-11

MINOR — **F26 + F32 SHIPPED: `skillet capture` — record a session as scrubbed, scored evidence** ([Specs/013](Specs/013-capture-session-evidence/plan.md) + [Specs/014](Specs/014-capture-secret-sanitization/plan.md); design → v0.51). `skillet capture --skill <s> --slug <x>` locates the newest claude-code session (or `--session <ref>`), normalizes it to a `Trace`, renders a transcript, captures the workspace `git diff` + produced file bodies, **redacts secrets in place** (`betterleaks`, resolved from flag/env/config/`PATH`; **fail-closed** if it can't run — constitution VI), runs the deterministic scorers over the redacted bodies, and writes the frozen bundle to `evaluations/<skill>/sessions/<date>-<slug>.*` (+ additive `.trace.json`). New libraries **`CorpusKit`** (bundle assembly + frozen writer) and **`SanitizerKit`** (`betterleaks` parser + span-merge redactor); `session-meta.json` → schema 2 with optional `sanitization` provenance; `--fail-on-secret` exits 1 for CI (bundle written scrubbed either way). Release-coupled — capture is exposure-gated on the real scanner, so there is never an un-scrubbed write path. The session-selection surface is `--session <ref>` overriding an implicit newest default (no `--last` flag — a no-op-default removed). 440 tests green; staged, pending commit. Phase 3 remains IN PROGRESS (F27/F28/F30/F31 remain).
